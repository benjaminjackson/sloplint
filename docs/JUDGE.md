...
- **positive-form** (`info`, `low`). Whether the sentence makes its claim by saying what is or by saying what is not: the claim rests on a negative (a denial, a correction with not, rather than or instead of, nothing or nobody as subject or object, a denial set up for the next sentence, an instruction not to do something), or the claim is positive, or its negative sits outside the claim (a condition, a bound, a fixed name, a standard's MUST NOT, quoted speech, or an assumption or decision that says so and names the option it rules out). Two levels. Flags at the first. Strunk and White's rule 15. The regex catalog's `not-x-but-y`, `isnt-x-its-y` and `not-nothing` see a few frames of this; this is the reading of where the negative sits. `info` because human prose denies as often as model prose: at the confidence `check` reports, 21 of 584 human news sentences flagged against 0 to 4 per 100 on the model sides (2 of 67 for Claude Sonnet 5), and 12 of 243 human abstract sentences against 1 of 163 for GPT-4 and 6 of 240 for Claude Sonnet 5; the rank statistic sits at 0.43 to 0.60. Off by default because reference documentation states its limits as negatives: in 48,000 words of gem READMEs it flagged 81 sentences and 14 of them were fair, the rest a limit the reader needs (not supported, does not retry, will not overwrite). `--select positive-form` or `--strict` runs it.
