...
## Calibration corpus: RAID and HAP-E, plus current models on their own titles
...
RAID has no opinion or essay writing, which is where the device and negation tics in this catalog live, so the `blogs` register reads from a second corpus instead: HAP-E, the Human-AI Parallel English Corpus (`browndw/human-ai-parallel-corpus` on Hugging Face, MIT). `script/calibrate fetch-hape` downloads it and writes `.corpus/hape/blogs.jsonl` and `.corpus/hape/blogs-chunk1.jsonl`, never committed. Each blog post is split into two chunks; every model side continues chunk 1 with the prompt "In the same style, tone, and diction of the following text, complete the next 500 words, generate exactly 500 words, and note that the text does not necessarily end after the generated words:" followed by the chunk, and the pairing `run` scores is that continuation against the human writer's own chunk 2, the same title continued twice. `script/calibrate generate MODEL --register blogs` writes a current model's continuation the same way, from chunk 1, into `.corpus/raid/generated/MODEL/blogs.jsonl` alongside the RAID-generated sides, so `run` and the gate treat it like any other register.

That is the whole calibration corpus. Earlier readings against Hacker News comments and IETF RFCs are retired: comments are not a register the tool is for, and RFCs paired against generated design documents were two genres, not a pair. The engineering register stays what it is for sloplint's regex rules, a reading discipline (CLAUDE.md), not a calibration gate. RAID and HAP-E are the floor for false positives; the generated side is the evidence about current models.
...
- `--strict` runs the low-confidence rules of both catalogs, the judge's `same-weight`, `stated-stakes`, `matched-shape`, `mirrored-opposites`, `positive-form`, `name-the-thing`, `stand-in-verb` and `dressed-pair` among them, runs the judge's sentence rules on every sentence, and keeps the judge's low-confidence notes.
...
check options:
  ...
  --markdown           skip fenced/inline code spans, HTML comments, and URLs; drop furniture that
                       needs a Markdown reading (front matter, indented code, a lone HTML or
                       code-span line). Headings, lists, tables, and quotes drop either way.
...
Twenty-one rules, two categories. A number appears only where it decides something a reader can see in the rule: its severity, its confidence ceiling, or a caveat. It is the probability that a model unit scores higher than a human one in the same register; below 0.5 means the rule ranks model prose lower, which is what a rule is for. The full runs, with corpus sizes and the models on each side, are in the commit that added or last changed the rule.
...
- **no-news** (`warning`, `high`). For the stated reader, the sentence explains what they already know, states what they could have guessed, or tells them something new. The explains-known level also catches a second-person sentence that reads the reader's own past actions or their own past words back to them: a fact they know because they did it or said it is not news, however specific it is. Flags at explains-known.
...
- **mirrored-opposites** (`info`, `low`). Two different claims joined by opposite words so they read as a contrast (everywhere and nowhere, always and never, every and none), or opposites that compare one thing across two cases, or no opposites. Two levels. Flags at the first. The narrow case of `matched-shape`. The flag is rare everywhere it was tested, against a current-model side: in news, 3 of 3,959 human sentences against 1 of 2,283 pooled across four model sides; in abstracts, 0 of 1,683 human against 0 of 4,951 model, on every side; both reverse or sit flat. In the HAP-E blogs register, where the essay and advice-writing tic this rule is meant for actually lives, it separates: 1 of 5,921 human sentences against 2 to 3 per roughly 2,500-to-6,000-sentence model side (GPT-4o, Llama 3 70B, Claude Sonnet 5). `info` because of the reversal in news and abstracts. Off by default because under `--strict` it reads a plain "X, but not Y" as a pair of opposites: 6 low-confidence hits in 55,000 words of gem READMEs, none fair, and none at the confidence `check` reports. `--select mirrored-opposites` or `--strict` runs it.
- **positive-form** (`info`, `low`). Whether the sentence makes its claim by saying what is or by saying what is not: the claim rests on a negative (a denial, a correction with not, rather than or instead of, nothing or nobody as subject or object, a verb that performs an absence such as publishes no or records nothing, a denial set up for the next sentence, an instruction not to do something), or the claim is positive, or its negative sits outside the claim (a condition, a bound, a fixed name, a standard's MUST NOT, quoted speech, or an assumption or decision that says so and names the option it rules out), or the absence is stated as a property of the thing (has no, without), or an expected behavior is denied on its own (does not retry), or a courtesy line releases the reader (no need to reply). Two levels. Flags at the first. Strunk and White's rule 15. The regex catalog's `not-x-but-y`, `isnt-x-its-y` and `not-nothing` see a few frames of this; this is the reading of where the negative sits. `info` because in news and abstracts human prose denies more often than model prose: at the confidence `check` reports, 20 of 584 human news sentences flagged against 0 to 4 per 100 on the model sides (2 of 67 for Claude Sonnet 5), and 12 of 243 human abstract sentences against 1 of 163 for GPT-4 and 6 of 240 for Claude Sonnet 5. Blogs is the one register where Claude Sonnet 5 denies more: 73 of 483 sentences against 106 of 964 human, 50 of 959 for GPT-4o and 90 of 966 for Llama 3 70B. The rank statistic sits at 0.42 to 0.60. Off by default because reference documentation states its limits as negatives: in 48,000 words of gem READMEs it flagged 47 sentences and 18 of them were fair, the rest a limit the reader needs (not supported, not encrypted, cannot be used with) or an instruction that names the option to use. It also flags an absence whose scope is named precisely enough to check ("Nobody on the team has reviewed the migration yet"), which a careful writer may fairly keep, and that is another reason it is off by default. `--select positive-form` or `--strict` runs it.
- **name-the-thing** (`info`, `low`). Whether the sentence calls things by their names or makes the reader map a pointer back to one: a position (the former, the latter, the second one, the last of three, the fourth item), even when the list was named a clause earlier; a stand-in noun or figure for a named thing (the engine for a named service, the seat for a named role); an allusion to a divide it never names ("where you stood when the old scheduler went away"); or a self-reference such as "this entire writeup" standing for a point it never states. A position that is the thing's own name (step 4, the second retry, Q3, the first argument), a which-clause about named things, and a pronoun pointing at something just named all pass. Two levels. Flags at the first. `the-x-is-not-the-x` is the repeated-noun case; this is the general one. `info` because it reverses in news: human sentences flag at 17 in 584, model sentences at 0 to 2 per side of about 100, and the human hits are the sports desk's epithet for a player named a sentence earlier ("the striker", "the Leeds-born sprinter"), which the rule is right to flag and which news does by convention. In abstracts 1 human and 0 to 1 model sentences flag. Off by default because in 31,000 words of gem READMEs it made 5 hits at the confidence `check` reports and 24 under `--strict`, and none of them was fair: an argument or a request whose position is its name, and a "this" or "these" pointing at the code beside it. `--select name-the-thing` or `--strict` runs it.
- **stand-in-verb** (`info`, `low`). Whether the sentence's main verb phrase is a figure of holding, carrying or keeping whose object cannot be held (holds the history of a job, carries the context on a service), standing in for a plain verb such as knows, wrote, decides or runs. A verb of holding or carrying in its literal or technical sense (holds a row lock, carries a header, owns the service) passes, and so does a negative subject with a plain verb (nobody on the team has reviewed the migration). Two levels. Flags at the first. `name-the-thing` is the same move made with a noun. `info` because it is rare in news and abstracts: 0 of 584 human news sentences and 0 of 243 human abstract sentences flag, against 0 to 2 per model side. In blogs 2 of 964 human sentences flag, one an idiom (hold in mind) and one a held that stands for had, against 17 of 959 for GPT-4o and 6 of 946 for Llama 3. Off by default because it has not been read against reference documentation, where holds, carries and keeps are ordinary technical verbs. `--select stand-in-verb` or `--strict` runs it.
- **maxim** (`info`, `medium`). Whether `target` is a saying, a general rule about how things always go stated in a turned phrase that could stand alone, or a fact about this system, team or event, how a particular tool, protocol or piece of code behaves, or a step, a requirement or a decision. Two levels. Flags at the first. `info` because the rule reverses in abstracts against 2023 models (rank statistic 0.52 to 0.58; it holds at 0.48 against Claude Sonnet 5 there), while it holds in news (0.37 to 0.50) and blogs (0.39 to 0.46). At the confidence `check` reports: news, human 0 of 584 against 0 to 5 per model side; abstracts, human 0 of 243 against 0 on every model side; blogs, where a saying is part of the register's own voice, human 37 of 964 against 13 to 43 per model side. Read against 30,600 words of gem READMEs, 0 hits. `medium` because the question turns on the sentence's shape, not a fixed list of words, and a plain general statement of fact ("adding a volatile-default column rewrites the table") has to be told apart from an aphorism built the same short, declarative way.
- **dressed-pair** (`info`, `low`). Whether the sentence says one fact as if it were two: a second clause that gives a second subject the first clause's verb and adds little beyond a tie back (X deploys and Y deploys with it), or a "both" or "those two things" summing up a pair set out earlier and handing the sum a verdict (is the real work, is the whole point). A pair whose halves carry different facts, a choice between two named options, a plain "both", and a pairing of groups with roles the paragraph defined all pass. Two levels. Flags at the first. `matched-shape` is any pair built to a rhythm and `name-the-thing` a pair pointed at by position or figure; this is the pair restated instead of named. `info` because no current model is caught doing it: at the confidence `check` reports, Claude Sonnet 5 flags 0 sentences in news, abstracts or blogs, against 5 of 964 human blog sentences, 12 of 959 for GPT-4o and 3 of 966 for Llama 3; news and abstracts flag at most 1 sentence a side. Read on a second draw, 2 of the 6 human blog hits are the device, a verb carried over to a second subject, and the rest are a pair of some other kind. Off by default because in 35,000 words of gem READMEs it made 2 hits at the confidence `check` reports and 13 under `--strict`, and none of them was this device. `--select dressed-pair` or `--strict` runs it.
- **device-before-claim** (`info`, `medium`). Whether the sentence makes its claim through a mirror, a riddle, a held-back answer, a coined figure or a maxim, or states it plainly. Two levels. Flags at the device. Ranks model prose lower in blogs against every side (0.34 to 0.43) and against the current model in news (0.39) and abstracts (0.44), but reverses against the 2023 models in news (0.51, 0.51) and abstracts (0.54 to 0.58). `info` because of that reversal. Flag counts at the confidence `check` reports: news, 37 of 584 human sentences against 8 to 26 of 67 to 133 model sentences; abstracts, 17 of 243 human against 8 to 37 of 155 to 240 model sentences; blogs, 104 of 964 human against 110 to 329 of 483 to 966 model sentences.
...
Paragraphs are separated by sloplint's `PARA_BREAK`. The judge drops headings, list items, table rows, block quotes, horizontal rules and reference lines whether or not `--markdown` is set, because a heading is not a paragraph and a bullet is not a sentence, and their line shape says so on its own. A heading written the setext way -- a title line followed by a line of nothing but `=` or two or more `-` -- drops both of its lines; only the underline is a horizontal rule when the line above it is blank, since a title has to sit right above its underline with nothing between them. A numbered line is a list item only where CommonMark says so: after a blank line or another piece of furniture, or when it starts at 1. A hard-wrapped prose line that happens to begin with a year stays prose. `--markdown` additionally uses sloplint's blanker, with one change: fenced code and HTML comments become spaces before splitting, as in the regex engine, but an inline code span or a URL becomes a run of one private-use character (U+E000) per character, so it keeps its length and cannot be mistaken for prose, because it is a word in its sentence. Blanked to spaces, a paragraph that opened with a code span lost its head and a sentence boundary followed by one merged two sentences; the placeholder keeps both, and a line that is only code or a bare link counts as furniture, but only under `--markdown` -- without it there is no placeholder to read, so that line is prose like any other. `--markdown` is also what turns YAML front matter, an indented code block and a lone HTML tag line into furniture; a document read without it keeps all three. Sentences are split on terminal punctuation followed by a space and a capital or an opening quote, with a short list of abbreviations that do not end a sentence, and a run of letters written one at a time (U.S., e.g.) does not end one either; a sentence that really ends on such a run merges with the next, and a lone capital before the period (Service A. Service B) still splits. This is the splitter the spike used, with one addition: every paragraph and sentence it returns carries its offset into the original text, so a note can be placed on the file as written after the furniture is gone. It lands in the core gem as `Sloplint::Split`, a public module the regex engine does not call, because the reader-test skill needs the same splitter and two consumers make it core's to own. A paragraph with fewer than three sentences is skipped by the paragraph rules and goes straight to the sentence rules, so a README written in two-sentence paragraphs is still examined. Splitting runs on the blanked copy, and every paragraph and sentence carries two strings cut at the same offsets: `text`, the span as the file has it, which is what a note quotes, so an excerpt is always a string that is in the file; and `asked`, the span with the furniture blanked, and, under `--markdown`, the HTML comments and fenced code blanked too, which is what the model is shown. Code spans and URLs stay in `asked`. Furniture is blanked line by line, so a list bound tight to the paragraph above it takes only its own lines out.
...
What the judge cannot fix is that shell. In Claude Code the agent that runs `sloplint check --judge` also has general Bash, so an instruction hidden in a document could still reach for `curl`. The judge keeps its own surface small: Jev answers with probabilities and a confidence, never free text, so nothing the model says reaches the agent as words, and the only document text that comes back is the note's `excerpt` and `context`: the flagged sentence or paragraph, with about forty characters either side, so a flagged paragraph does come back whole. Anyone who wants a harder line draws it in Claude Code's own settings, for example `permissions.deny` entries for `Bash(curl *)` and `Bash(wget *)`, or a subagent of their own with a PreToolUse hook that allows one command. A plugin cannot ship that clamp: plugin subagents can drop tools, not restrict Bash to one command line.

### Adding a backend

1. Write a class under `lib/sloplint/judge/backends/` that answers `ask` and `name`, and `cost_usd(usage)` if the backend has a list price, as a class method as well as an instance method, so the run that asks nothing can print a price of zero. Give it a `KEY` constant naming the environment variable its key lives under, which is also its keychain account under the `sloplint-judge` service, so two backends' keys are two items; a class method `model_name` that says what a built backend would answer `name` with, from the environment alone, so a run that asks nothing can report the same identifier; and a class method `configured!` that checks its endpoint settings without reading the key, raises `ArgumentError` when they are wrong, and returns a short description for `status`. The constructor takes `key: nil` and resolves it through `Secret.fetch(KEY)`.
2. Add its name to the backend table.
3. Run `script/calibrate` against the corpus described above: RAID's human side against both its 2023 generations and a current-model side generated from the same prompts. It reports, per rule, register and model side, the probability that the model unit scores higher than the human one, and how many units on each side it would have flagged at the confidence `check` reports; the flip rate under passage-order swap at each confidence band for `compare`; and the agreement rate between two runs of the same units at each band for `check`. A backend is usable when `no-news` and `names-nothing` sit below 0.5 in news against the current-model side, the `compare` flip rate at or above 0.7 confidence is under 1 percent over at least 100 pairs, and the `check` agreement rate at or above 0.7 is at least 90 percent. Abstracts is calibrated and reported but is not a pass line: an abstract a current model writes from a RAID title is, by the judge's measure, about as concrete as the human abstract for the same title, 0.50 to 0.54 on those two rules against Opus 5 and Fable 5.1, which is a fact about abstracts rather than about the backend. Under 100 pairs the flip line says so instead of failing, because under 1 percent of 48 pairs is no flip at all. If a backend's bands differ from Jev's, the confidence thresholds are per backend.
