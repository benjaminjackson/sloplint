...
- `--strict` runs the low-confidence rules of both catalogs, the judge's `same-weight`, `stated-stakes`, `matched-shape`, `mirrored-opposites` and `positive-form` among them, runs the judge's sentence rules on every sentence, and keeps the judge's low-confidence notes.
