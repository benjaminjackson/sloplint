...
- `trailing-restatement` — comma plus "which means", "which is to say" or the participle "meaning", or one of the result frames the neutral participles take ("making it easier", "allowing us to", "giving them more", "leaving you with"). A quote mark, underscore or bracket before the comma marks a gloss and is skipped; the noun and intention senses of "meaning" are out by the following word; the bare participles ("leaving the door open") never match. Off by default: the pattern sees the connective, not whether the tail restates the head. `info`.
