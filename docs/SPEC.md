...
- `the-x-is-not-the-x` — "the X … is not the X …": any head noun on both sides of a negated copula (is not, isn't, was not, were not); backreference; the clause between is capped at 50 chars and may not hold sentence punctuation, a comma, semicolon or colon; "of" after the second head and "the same X" are out, since both name the difference; the-x-is-the-x's abstract heads are skipped so each sentence is reported once; `medium` confidence
...
- `negative-subject` — a negative pronoun (Nothing, Nobody, No one, None) as the subject of a clause, where the clause also carries one of two structural markers rather than stopping at a bare subject and verb ("Nothing happened.", "No one knows." are not matched; a regex cannot tell a lexical verb from any other word, so the narrowing comes from what surrounds the subject). A clause starts at the beginning of the text, after `.;:!?` or a line break, or after a coordinating conjunction (and, but, or, so, yet, nor) — the one addition to `actually-not-x`'s clause-start set, since a conjunction can open a second clause inside one sentence; the other frame stays off the conjunction start, since "and" more often joins two noun phrases in one clause than two clauses. Either frame's clause-start may also open on one or two markdown emphasis characters (`*` or `_`), so a sentence set off in bold or italics is still read from its first word. What ends a frame is a full stop, or a clause break (a comma before a coordinating conjunction, or a bare comma, semicolon or colon): a flagged clause need not be the sentence's last. The two frames: two negative-subject clauses joined by "and", the first predicate's word count excluding "and" itself so an atomic group can't swallow the separator ("Nothing is deployed and nothing is queued."); a negative-subject clause whose object carries its own negated relative clause, the negation the full closed set of auxiliaries and contractions — does/do/did, has/have/had, is/are/was/were — with the first predicate excluding "that" for the same atomic-group reason as frame 1, and up to ten words between the negation and the clause end so a longer relative clause still closes the match ("nothing gets merged that does not get reviewed."); `medium` confidence. Two other frames were probed and cut: "No" plus a common noun ("No vendor holds a contract.") is indistinguishable from ordinary negation ("No installation is necessary.") and fired at or above the model rate on RAID; a bare pronoun with a prepositional phrase before the verb ("Nobody on the on-call page owns the service.") is indistinguishable from a real, specific absence with the PP naming its scope ("Nobody on the team has reviewed the migration yet.").
