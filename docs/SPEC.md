...
| `is-real-and-not` | "the X is real, and/not…", "is the real … and it", "no real X, but a real Y" | skip "real estate/time"; the "no real X, but a real Y" branch caps the noun phrase after the first "real" at three words; `info` |
