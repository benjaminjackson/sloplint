...
- `trailing-restatement` — comma plus "which means", "which is to say", or "meaning" opening on a determiner, a pronoun or "that"; or one of four participle frames with a pronoun object and a closing word ("making it easier", "allowing us to", "giving them more", "leaving you with"). An emphasis marker, backtick, quote or space before the comma marks a gloss or a blanked code span and is skipped; the bare participles ("leaving the door open") never match. Off by default: the pattern sees the connective, not whether the tail restates the head. `info`.
