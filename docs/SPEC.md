...
| `phrase-echo` | the same three words again within 400 words | backreference in a lookahead, so the match is the first occurrence; each word 4+ letters and lowercase-led, one of them 6+, so function-word runs, proper nouns and Title Case headings are out; a quoted repeat is out; the gap crosses paragraph breaks; off by default; `info` |
