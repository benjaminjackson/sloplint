...
| `the-x-is-the-x` | "the X … is the X …": the same abstract head noun on both sides of the copula | backreference; closed noun list; the clause between is capped at 50 chars and may not cross a semicolon; the second head must be followed by a preposition, determiner, pronoun or punctuation, so compounds are out |
