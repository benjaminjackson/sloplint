...
| `thats-the-whole` | "that/this is the whole point/game/thing…" | also value and fix, the closers agents write in technical prose, when they end the sentence or the line or run into a preposition; either apostrophe; `is-the-whole-x` yields the same list |
...
- `mic-drop-closer` — a sentence of 60+ characters, then a paragraph-final closer of two to eight words opening on a quantifier or deictic (Nothing, Most, None, Everything, Then, That, This, It…): "Nothing here needs a new login."; `info`. A blank line or the end of the text must follow the closer; both sentences may be hard-wrapped; whitespace runs in the long sentence are capped so blanked Markdown cannot make it. The note points at the closer. One means nothing; a draft where it repeats is the tell.
- `short-run` — three consecutive sentences of thirty characters or fewer, each letter-led and closing on a full stop, no quotation marks or digits, no lone-capital initials or abbreviations, starting at a real sentence boundary (never on a wrap continuation), crossing a hard wrap but not a paragraph break; a bullet may open the run, consecutive bullets are a list; `info`. One is a question; a draft that repeats it is the tell.
