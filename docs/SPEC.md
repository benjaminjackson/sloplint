...
- `short-run` — three consecutive sentences of thirty characters or fewer, each letter-led and closing on a full stop, no quotation marks or digits, no lone-capital initials, not crossing a paragraph break; `info`. One is a question; a draft that repeats it is the tell.
