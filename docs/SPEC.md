...
- `negative-subject` — a negative pronoun (Nothing, Nobody, No one, None) as the subject of a clause, where the clause also carries one of three structural markers rather than stopping at a bare subject and verb ("Nothing happened.", "No one knows." are not matched; a regex cannot tell a lexical verb from any other word, so the narrowing comes from what surrounds the subject). A clause starts at the beginning of the text, after `.;:!?` or a line break, or after a coordinating conjunction (and, but, or, so, yet, nor) — the one addition to `actually-not-x`'s clause-start set, since a conjunction can open a second clause inside one sentence; the other two frames stay off the conjunction start, since "and" more often joins two noun phrases in one clause than two clauses. The three frames: two negative-subject clauses joined by "and" ("Nothing is deployed and nothing is queued."); a bare pronoun with a prepositional phrase before the verb, "of" excluded since "none of this/that/it" is the ordinary partitive idiom ("Nobody on the on-call page owns the service."); a negative-subject clause whose object carries its own negated relative clause ("nothing gets merged that does not get reviewed."); `medium` confidence. A fourth frame, "No" plus a common noun ("No vendor holds a contract."), was probed and cut: it is indistinguishable from ordinary negation ("No installation is necessary.") and fired at or above the model rate on RAID.
