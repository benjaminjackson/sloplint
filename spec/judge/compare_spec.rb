...
RSpec.describe Sloplint::Judge::Compare do
  ...
  # `sloplint-judge compare` turns a BackendError into exit 3. A bare
  # KeyError here would have ended the command with a backtrace instead.
  it "calls a keep answer that is not a probability for A and one for B a backend failure" do
    keyed_wrong = FakeBackend.new { |_s, _n, _q| [{ "first" => 0.9, "second" => 0.1 }, 0.9] }
    expect { described_class.run("a", "b", place: "x", backend: keyed_wrong, swap: false) }
      .to raise_error(Sloplint::Judge::BackendError, /no probability for B/)
    not_a_hash = FakeBackend.new { |_s, _n, _q| [[0.9, 0.1], 0.9] }
    expect { described_class.run("a", "b", place: "x", backend: not_a_hash, swap: false) }
      .to raise_error(Sloplint::Judge::BackendError, /not a probability for A/)
    silent = Class.new do
      def name = "silent"
      def ask(*) = {}
    end.new
    expect { described_class.run("a", "b", place: "x", backend: silent, swap: false) }
      .to raise_error(Sloplint::Judge::BackendError, /answered no keep question/)
  end
